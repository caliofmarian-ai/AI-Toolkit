...
class EngineeringDashboardService:
    ...
    def _owner_ai_chat_panel(
        ...
        return (
            ...
            '<button id="chat-continue" type="button">'
            'Continue interrupted turn</button>'
            '</form>'
            '</div>'
            '<script>'
            '(()=>{'
            ...
            'if(!session.value){status.className="chat-status error";'
            'status.textContent="Select the interrupted conversation first.";'
            'return;}'
            'continueButton.disabled=true;send.disabled=true;'
            'question.disabled=true;'
            'status.className="chat-status working";'
            'status.textContent="AI Partner is continuing the preserved turn…";'
            'try{const data=await jsonFetch("/api/ai/chat",{'
            'method:"POST",headers:{"Content-Type":"application/json"},'
            'body:JSON.stringify({question:"",session_id:session.value,'
            'provider_id:provider.value,model:model.value,'
            'resume_interrupted_turn:true})});'
            'const sid=data.session_id||session.value;'
            'if(sid){session.value=sid;localStorage.setItem('
            '"ai_toolkit_partner_session_id",sid);}'
            'await loadSessions(sid);session.value=sid;'
            'await loadSession();'
            'status.className="chat-status";'
            'status.textContent="Interrupted turn continued and persisted.";}'
            'catch(e){status.className="chat-status error";'
            'status.textContent=e.message;}finally{'
            'continueButton.disabled=false;send.disabled=false;'
