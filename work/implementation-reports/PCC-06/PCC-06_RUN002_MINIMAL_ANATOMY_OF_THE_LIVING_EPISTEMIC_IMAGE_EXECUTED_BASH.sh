...
if [ "$CURRENT_BRANCH" != "$BRANCH" ]; then
    echo "STOP: recovery launcher did not establish expected transformation branch."
...
# The execution record is a metabolic product of this RUN.
# Normalize only the new execution-record copy before Git whitespace
# examination. The historical recovery body remains untouched in its
# recovery commit.
python - <<'PY_INNER'
from pathlib import Path

path = Path("work/implementation-reports/PCC-06/PCC-06_RUN002_MINIMAL_ANATOMY_OF_THE_LIVING_EPISTEMIC_IMAGE_EXECUTED_BASH.sh")

if not path.is_file():
    raise SystemExit("STOP: execution record disappeared before conservation")
