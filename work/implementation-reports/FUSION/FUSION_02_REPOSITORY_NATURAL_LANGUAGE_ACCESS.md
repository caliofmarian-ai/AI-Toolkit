# FUSION-02 — Natural-Language Repository Access Completion

## Human meaning

The AI Partner already possessed physical access to the repository deployed
with its Railway runtime.

The missing physiology was the ability to transform a natural-language human
question into bounded repository candidates.

The existing EvidenceEngine has now been extended rather than replaced.

## Demonstrated anatomy

The repository path remains the local repository_root.

EvidenceEngine traverses that root.

SemanticEngine remains part of discovery.

AIPlatformService continues to perform bounded repository reads.

No GitHub connector, OAuth flow, token, network filesystem, or second
repository organ was introduced.

## Correction

Natural-language repository questions are tokenized into meaningful search
terms.

Repository-relative paths and filenames are ranked using those terms.

Generic conversational words do not cause repository-wide selection.

Exact filename discovery remains supported.

The resulting candidates continue through the existing cognitive navigation,
bounded read, Working Context, Context Budget, and provider physiology.

## Acceptance

- real FUSION-02 tree discovered from natural language;
- real AI Platform service file discovered from natural language;
- discovered files physically read from repository_root;
- bounded path authority conserved;
- complete FUSION regression passed;
- Human Authority architecture unchanged.

## FUSION-02 status

FUSION-02 remains structurally closed at E0-E20.

This correction completes a demonstrated runtime-access defect in the
already-certified physiology. It does not invent E21.

## Complete conserved FUSION-02 tree
...
## [x] E19 — Comportamentul organismului la eroare, oprire și repornire

└── [x] T15 — Failure / Restart / Human Authority Validation
...
├── [x] interruption checkpoint
├── [x] current Journey identity conserved
├── [x] current Need identity conserved
├── [x] interruption status conserved
├── [x] stopping reason conserved
├── [x] restart-recoverable checkpoint
├── [x] checkpoint survives engine restart
├── [x] provider failure confers no authority
├── [x] Human Authority preserved
├── [x] no Journey fabrication without Journey
├── [x] no AI answer fabrication on provider failure
├── [x] no usage fabrication on provider failure
├── [x] new Journey allowed after interruption
├── [x] unknown persistent session fails closed
└── [x] synthetic service boundary remains non-persistent

### Arbore erori E19

├── [R] E19-ERR-001 — PYTHON_SOURCE_LITERAL_NEWLINE_ESCAPE_CORRUPTION
│   └── [x] recovered without reset
├── [R] E19-ERR-002 — INCOMPLETE_SYNTHETIC_SESSION_EXPERIENCE_BOUNDARY_FIXTURE
│   └── [x] recovered without weakening production
├── [R] E19-ERR-003 — INCOMPLETE_SYNTHETIC_SESSION_CONTEXT_RECONSTRUCTION_FIXTURE
│   └── [x] recovered without weakening production
└── [R] E19-ERR-004 — RECOVERY_AUDIT_GATE_FORMAT_ASSUMPTION
    └── [x] recovered by semantic worktree inspection

Stare E19/T15:
FINALIZAT / CERTIFICAT
## [x] E20 — Retragerea fiziologiei vechi ca mecanism implicit

└── [x] T16 — Legacy Default Context Retirement
...
Erori istorice E20/T16:

└── [!] E20-ERR-001 — LEGACY_RETIREMENT_COMPATIBILITY_BOUNDARY_REGRESSION
    └── [R] legacy a fost retras din fiziologia implicită a serviciului real,
        fără eliminarea compatibilității istorice din pipeline
